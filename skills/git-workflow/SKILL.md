---
name: git-workflow
description: This skill should be used when the user asks to "create git commit", "manage branches", "follow git workflow", "use Conventional Commits", "handle merge conflicts", "version control for research", "git for data analysis", "how to track analysis scripts", "version analysis code", "commit Stata do files", "commit Python scripts", "research project git structure", or asks about git branching strategies, version control best practices, pull request workflows.
version: 2.0.0
---

# Git Workflow Standards

Covers both **research project version control** (data analysis, empirical research) and general software development Git standards.

---

## Part 1: Research Project Version Control
...
research-project/
├── .git/
├── .gitignore              ← critical: what to exclude
├── .dvc/                   ← DVC metadata (commit this)
├── README.md               ← project overview + run instructions
│
├── data/
│   ├── raw/                ← NEVER commit; track with DVC or git-lfs
│   │   ├── census_2020.dta
│   │   └── README.md       ← document sources, access, download
│   ├── processed/          ← derived data; git-ignore or DVC
│   └── census_2020.dta.dvc ← DVC pointer file (commit this)
│
├── analysis/
│   ├── 00_setup.do/.py     ← environment + package setup
│   ├── 01_clean.do/.py     ← data cleaning
│   ├── 02_main_reg.do/.py  ← main specifications
│   ├── 03_robustness.do/.py
│   ├── 04_figures.do/.py
│   └── 05_tables.do/.py
│
├── output/
│   ├── figures/            ← generated; can git-ignore or track
...
│
└── config/
    └── paths.do/.py        ← project-wide path configuration
```

### Research Commit Scopes

| Scope | What it covers | Example |
|-------|---------------|---------|
| `data` | Data acquisition, cleaning scripts | `feat(data): add county-level census panel 2000-2020` |
| `analysis` | Regression specs, estimation code | `feat(analysis): implement 2×2 DID with state FE` |
| `results` | Output files, tables, figures | `update(results): regenerate Table 2 with alt clustering` |
| `paper` | LaTeX writing | `feat(paper): draft identification section` |
| `config` | Paths, globals, settings | `fix(config): correct output directory paths` |
| `repkg` | Replication package preparation | `chore(repkg): add AER replication README` |

### Research Branch Strategy

```
master          ← publication-ready / submission version
develop         ← working integration branch
│
├── analysis/main-spec        ← main identification strategy
├── analysis/robustness       ← robustness checks & extensions
├── analysis/hetero           ← heterogeneity analysis
├── paper/draft               ← active writing
├── paper/revision-r1         ← first revision
...
# ── Output (derived; regenerate from code) ──────────
output/
results/
...
dvc init                     # creates .dvc/ in project root
git add .dvc .dvcignore
git commit -m "chore(config): initialize DVC"
```

**Track large data files:**
```bash
# Add file to DVC (creates .dvc pointer file)
dvc add data/raw/census_2020.dta

# Commit the pointer (not the data itself)
git add data/raw/census_2020.dta.dvc data/raw/.gitignore
git commit -m "data(raw): add census 2020 panel via DVC"

# Configure remote storage
dvc remote add -d myremote ~/dvc-storage   # local disk
# or: dvc remote add -d s3remote s3://mybucket/dvc
...
**Alternative: git-lfs (simpler, for <2GB files):**
```bash
git lfs install
git lfs track "*.dta" "*.csv" "*.parquet"
git add .gitattributes
git commit -m "chore(config): enable git-lfs for data files"
git add data/raw/census_2020.dta
git commit -m "data(raw): add census 2020 panel"
```

### Research Commit Workflow (Daily)
...
# Work on analysis
# Edit: analysis/02_main_reg.py

# Stage and commit analysis changes
git add analysis/02_main_reg.py
git commit -m "feat(analysis): add state×year FE to main DID spec"

# If data changed
dvc add data/processed/panel_clean.dta
git add data/processed/panel_clean.dta.dvc
git commit -m "data(processed): update cleaned panel with income vars"
dvc push

# Update results
git add output/tables/table2.tex output/figures/event_study.pdf
git commit -m "update(results): regenerate Table 2 and event study plot"
```

### Tagging Analysis Milestones

```bash
# Tag at key milestones (submission, revision, etc.)
git tag -a v1.0-submission -m "First submission to APSR (2026-03-14)"
git tag -a v1.1-revision-r1 -m "Revision R1 response submitted"
